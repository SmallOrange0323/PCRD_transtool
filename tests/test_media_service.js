...
// Case 1: 正常 voiceName 解析 (7-14 位為 groupId)
{
    ...
    assert.strictEqual(candidates[0], 'https://pcrd-story-voice.pages.dev/story_vo/1001001/vo_adv_1001001_001.m4a', '第 1 順位必須為 Cloudflare Pages');
    assert.strictEqual(candidates[1], 'https://prcn-sound.estertion.win/story_vo/1001001/vo_adv_1001001_001.m4a', '第 2 順位必須為 PRCN');
    assert.strictEqual(candidates[2], 'https://redive.estertion.win/sound/story_vo/1001001/vo_adv_1001001_001.m4a', '第 3 順位必須為 REDIVE');
    assert.ok(!candidates.some(url => url.startsWith('sound/story_vo/')), '不得產生 local sound/story_vo URL');
}
...
// Case 2: 另一種 voiceName 格式
{
    ...
    assert.strictEqual(candidates[0], 'https://pcrd-story-voice.pages.dev/story_vo/5045001/vo_adv_5045001_005.m4a');
    ...
    assert.ok(!candidates.some(url => url.startsWith('sound/story_vo/')));
}
