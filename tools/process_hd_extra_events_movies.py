# -*- coding: utf-8 -*-
"""
tools/process_hd_extra_events_movies.py
新形式活動 (extra_events.json: 10201 ~ 10216) PC (DMM) 高畫質動畫 1080p 升頻與繁中字幕壓制管線
"""
...
DEFAULT_OUT_BASE = ROOT / "downloads" / "movies" / "extra_events_hd_subtitled"
EXTRA_EVENTS_JSON = ROOT / "dashboard" / "data" / "extra_events.json"
...
Title: PCRD Extra Event Movie Official Subtitle
ScriptType: v4.00+
PlayResX: 1920
PlayResY: 1080
WrapStyle: 0
...
def get_extra_events():
    data = json.load(open(EXTRA_EVENTS_JSON, encoding="utf-8"))
    events = data.get("events", [])
    event_map = {}
    for e in events:
        gid = e.get("story_group_id")
        title = sanitize_folder_name(e.get("title", ""))
        sub_tag = str(gid)[-3:]
        prefix_5 = f"5{sub_tag}"
        event_map[gid] = {
            "title": title,
            "prefix_5": prefix_5,
            "gid_str": str(gid)
...
def get_manifests(ver, event_map):
    ...
    for line in data_pc.splitlines():
        p = line.strip().split(",")
        if len(p) >= 5 and p[0].startswith("m/"):
            fn = Path(p[0]).name
            for gid, info in event_map.items():
                if info["gid_str"] in fn or f"story_{info['prefix_5']}" in fn:
    ...
    return matched_movies, sub_bundles
...
def process_movie(item, sub_bundles, out_dir, ffmpeg_exe, force=False):
    fn = item["filename"]
    num_m = re.search(r'(\d+)', fn)
    base_id = num_m.group(1) if num_m else Path(fn).stem
...
def main():
    parser = argparse.ArgumentParser(description="PCRD 新形式活動 (extra_events.json) PC 高畫質 1080p 動畫壓制工具")
    ...
    ver = _get_sonet_ver()
    print("==================================================")
    print("🎬 PCRD 新形式活動 (extra_events.json) 1080p 原生字體壓制管線")
    print("==================================================")
    print(f"[CDN] 台服 TruthVersion: {ver}")
    print("[Manifest] 正在獲取新形式活動清單與台版字幕對照表...")

    event_map = get_extra_events()
    matched_movies, sub_bundles = get_manifests(ver, event_map)

    total_count = len(matched_movies)
    total_raw_sz = sum(m["size_bytes"] for m in matched_movies)

    DEFAULT_OUT_BASE.mkdir(parents=True, exist_ok=True)

    print(f"[目標] 涵蓋 extra_events.json 共 {len(event_map)} 個新形式活動，檢索到 {total_count} 部動畫")
    ...
    for idx, item in enumerate(matched_movies, 1):
        gid = item["group_id"]
        ...
        print(f"[{idx:3d}/{total_count:3d}] [{gid}] {title[:16]:<16} | {fn:<28} ({sz_mb:>6.2f} MB)... ", end="", flush=True)
    ...
    print("\n==================================================")
    print(f"🎉 新形式活動批次壓制結束！(總耗時: {elapsed/60:.1f} 分鐘)")
